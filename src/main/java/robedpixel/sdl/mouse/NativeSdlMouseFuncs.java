...
class NativeSdlMouseFuncs {
        ...
        SDL_WarpMouseGlobal =
                Linker.nativeLinker()
                        .downcallHandle(
                                ...
                                FunctionDescriptor.of(ValueLayout.JAVA_BOOLEAN, ValueLayout.JAVA_FLOAT,ValueLayout.JAVA_FLOAT));
    ...
    public synchronized void  getGlobalMouseState(SdlMouseState state) throws Throwable {
        synchronized (addressMutex) {
            int mouseButtonFlags = (int) SDL_GetGlobalMouseState.invoke(tempFloatAddress, temp2FloatAddress);
            state.setX(tempFloatAddress.get(ValueLayout.JAVA_FLOAT, 0));
            state.setY(temp2FloatAddress.get(ValueLayout.JAVA_FLOAT, 0));
            state.setMouseButtonFlags(mouseButtonFlags);
        }
    ...
    public void getRelativeMouseState(SdlMouseState state) throws Throwable {
        synchronized (addressMutex) {
            int mouseButtonFlags = (int) SDL_GetRelativeMouseState.invoke(tempFloatAddress, temp2FloatAddress);
            state.setX(tempFloatAddress.get(ValueLayout.JAVA_FLOAT, 0));
            state.setY(temp2FloatAddress.get(ValueLayout.JAVA_FLOAT, 0));
            state.setMouseButtonFlags(mouseButtonFlags);
        }
    }
    public synchronized void warpMouseInWindow(MemorySegment window,float x, float y) throws Throwable {
    ...
    public synchronized boolean setRelativeMouseTransform(MemorySegment callback, MemorySegment userdata) throws Throwable {
        return (boolean) SDL_SetRelativeMouseTransform.invoke(callback,userdata);
    }
    public synchronized boolean  setWindowRelativeMouseMode(MemorySegment window, boolean enabled) throws Throwable {
    ...
    public synchronized MemorySegment  createCursor(MemorySegment data, MemorySegment mask, int w, int h, int hot_x, int hot_y) throws Throwable {

...
class NativeSdlSensorsFuncs {
    ...
    public NativeSdlSensorsFuncs(Arena allocator) {
        ...
        SDL_GetSensorTypeForID = Linker.nativeLinker().downcallHandle(
                library.find("SDL_GetSensorTypeForID").orElseThrow(),

                FunctionDescriptor.of(ValueLayout.JAVA_INT, ValueLayout.JAVA_INT)
        );
        SDL_GetSensorNonPortableTypeForID = Linker.nativeLinker().downcallHandle(
                library.find("SDL_GetSensorNonPortableTypeForID").orElseThrow(),

                FunctionDescriptor.of(ValueLayout.JAVA_INT, ValueLayout.JAVA_INT)
        );
        SDL_OpenSensor = Linker.nativeLinker().downcallHandle(
                library.find("SDL_OpenSensor").orElseThrow(),

                FunctionDescriptor.of(ValueLayout.ADDRESS, ValueLayout.JAVA_INT)
        );
        SDL_GetSensorFromID = Linker.nativeLinker().downcallHandle(
                library.find("SDL_GetSensorFromID").orElseThrow(),

                FunctionDescriptor.of(ValueLayout.ADDRESS, ValueLayout.JAVA_INT)
        );
        SDL_GetSensorProperties = Linker.nativeLinker().downcallHandle(
                library.find("SDL_GetSensorProperties").orElseThrow(),

                FunctionDescriptor.of(ValueLayout.JAVA_INT, ValueLayout.ADDRESS)
        );
        SDL_GetSensorName = Linker.nativeLinker().downcallHandle(
                library.find("SDL_GetSensorName").orElseThrow(),

                FunctionDescriptor.of(ValueLayout.ADDRESS, ValueLayout.ADDRESS)
        );
        SDL_GetSensorType = Linker.nativeLinker().downcallHandle(
                library.find("SDL_GetSensorType").orElseThrow(),

                FunctionDescriptor.of(ValueLayout.JAVA_INT, ValueLayout.ADDRESS)
        );
        SDL_GetSensorNonPortableType = Linker.nativeLinker().downcallHandle(
                library.find("SDL_GetSensorNonPortableType").orElseThrow(),

                FunctionDescriptor.of(ValueLayout.JAVA_INT, ValueLayout.ADDRESS)
        );
        SDL_GetSensorID = Linker.nativeLinker().downcallHandle(
                library.find("SDL_GetSensorID").orElseThrow(),

                FunctionDescriptor.of(ValueLayout.JAVA_INT, ValueLayout.ADDRESS)
        );
        SDL_GetSensorData = Linker.nativeLinker().downcallHandle(
        ...
        SDL_CloseSensor = Linker.nativeLinker().downcallHandle(
                library.find("SDL_CloseSensor").orElseThrow(),

                FunctionDescriptor.ofVoid(ValueLayout.ADDRESS)
        );
        SDL_UpdateSensors = Linker.nativeLinker().downcallHandle(
                library.find("SDL_UpdateSensors").orElseThrow(),

                FunctionDescriptor.ofVoid()
        );
    }
    public synchronized MemorySegment getSensors(int count) throws Throwable {
        tempIntAddress.set(ValueLayout.JAVA_INT,0,count);
        return (MemorySegment) SDL_GetSensors.invoke(tempIntAddress);
    }
    ...
    public synchronized int getSensorData(MemorySegment sensor,MemorySegment data,int numValues) throws Throwable {
        return (int)SDL_GetSensorData.invoke(sensor,data,numValues);
    }
    ...
    public static NativeSdlSensorsFuncs getInstance(Arena allocator) {
        ...
        if (result == null) {
            synchronized (mutex) {
                result = INSTANCE;
                if (result == null)
                    INSTANCE = result = new NativeSdlSensorsFuncs(allocator);
            }
        }
        return result;
    }

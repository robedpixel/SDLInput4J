...
public class SdlTextEditingCandidatesEvent {
  ...
  public static SdlTextEditingCandidatesEvent getEventFromMemorySegment(MemorySegment segment) {
    ...
    if (numCandidates > 0) {
      ...
      MemorySegment stringSegmentArray =
          ((MemorySegment) candidatesHandle.get(segment, 0))
              .reinterpret(ValueLayout.ADDRESS.byteSize() * numCandidates);
      for (int i = 0; i < numCandidates; i++) {
        MemorySegment stringSegment = stringSegmentArray.getAtIndex(ValueLayout.ADDRESS, i);
        retEvent.candidates[i] =
            stringSegment.reinterpret(Integer.MAX_VALUE).getString(0, StandardCharsets.US_ASCII);
      }
    }
